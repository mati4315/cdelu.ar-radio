#!/bin/bash
# Setup Conky monitor for radioadmin on xrdp display :10

CONKYRC="/home/radioadmin/.conkyrc"
AUTOSTART_DIR="/home/radioadmin/.config/autostart"
...
conky.text = [[
${color1}${font DejaVu Sans Mono:size=14:bold} RADIO VPS - CDELU${font}
${color2}${hr}
${color1}SISTEMA
...
${color1}${hr}
${color2}${alignc}${font DejaVu Sans Mono:size=8}${time %d/%m/%Y %H:%M:%S}${font}
]];
CONKYEOF

# Create autostart
mkdir -p "$AUTOSTART_DIR"
cat > "$AUTOSTART_DIR/conky.desktop" << 'DESKTOPEOF'
[Desktop Entry]
Type=Application
Name=Conky Monitor
Exec=env DISPLAY=:10 conky -d
StartupNotify=false
Hidden=false
X-XFCE-Autostart-Enabled=true
DESKTOPEOF

chown -R radioadmin:radioadmin /home/radioadmin/.config

# Kill existing conky and start as radioadmin
killall conky 2>/dev/null
su - radioadmin -c 'export DISPLAY=:10 && conky -d' 2>&1
